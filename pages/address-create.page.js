...
class AddressCreatePage extends BasePage {
  ...
  get addressInput() {
    return new Input(
      $('(//textarea[@data-placeholder="Please provide an address."])'),
      "Address Input"
    );
  }
  ...
  get invalidMobileNumberMsg() {
    return new Label(
      $(
        '//mat-error[contains(text(),"Mobile number must match 1000000-9999999999 format.")]'
      ),
      " Mobile number input is not valid Massege."
    );
  }

  get emptyMobileNumberMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide a mobile number.")]'),
      " Mobile number input is empty Massege."
    );
  }

  get emptyContryMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide a country.")]'),
      " Contry input is empty Massege."
    );
  }

  get emptyNameMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide a name.")]'),
      " Name is empty Massege."
    );
  }

  get emptyZipMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide a ZIP code.")]'),
      " Zip Code is empty Massege."
    );
  }

  get emptyAddressMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide an address.")]'),
      " Address input is empty Massege."
    );
  }

  get emptyCityMsg() {
    return new Label(
      $('//mat-error[contains(text(),"Please provide a city.")]'),
      " City input is empty Massege."
    );
  }
