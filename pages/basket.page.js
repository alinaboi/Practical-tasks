...
const REMOVE_BTN_LOCATOR =
  '/following-sibling::mat-cell[@class="mat-cell cdk-cell cdk-column-remove mat-column-remove ng-star-inserted"]';
...
class BasketPage extends BasePage {
  ...
  async getProductByText(productText) {
    return new Label(
      $(PRODUCT_TEXT_LOCATOR(productText)),
      `${productText} Text`
    );
  }
  ...
  async getProductByBegianningOfText(productText) {
    return new Label(
      $(PRODUCT_STARTS_WITH_TEXT_LOCATOR(productText)),
      `${productText} Text`
    );
  }
  ...
  async clickRemoveByProductText(productText) {
    let removeButton = new Button(
      $(PRODUCT_TEXT_LOCATOR(productText) + REMOVE_BTN_LOCATOR),
      "Remove the ithem from Basket"
    );
