...
describe('API testing', async () => {
    ...
    it('Add to basket via API', async () => {
        ...
        let addToBasket = await (await Api.basketApi()).addToBasket(bId, data.productId, data.productQuantity);
        await Api.verifyStatusIsSuccess(addToBasket);
    });

    it('Add delivery address via API', async () => {

        let addAddress = await (await Api.addressApi()).addAddress(data.address);
    ...
    it('Add payment card via API', async () => {
        ...
        let addCard = await (await Api.cardApi()).addCard(data.paymentMethod);
    ...
    it('Final checkout via API', async () => {
        ...
        let checkoutResponse = await (await Api.checkoutApi()).checkout(bId, addressId, data.deliveryMethod, paymentId);
